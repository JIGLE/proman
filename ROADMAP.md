...
> audit and `docs/DEVELOPMENT_ROADMAP.md` for what's planned next).
...
| 2026-07-09 | IA reconciled to what's actually live (Operations/Intelligence-as-Reports/System), not the roadmap's original "task-oriented" framing (roadmap 3.3) | Three competing nav proposals existed: `docs/UI_CONSISTENCY_GUIDE.md` (flat Assets/People/Finance), `docs/UX_AUDIT_2026.md` (Operations/Intelligence/System), and `docs/UX_IMPROVEMENT_PLAN.md` §3.1 (task-oriented: "Collect Rent", etc.). Checked the actual code before executing the roadmap's literal instruction to adopt "task-oriented" — `docs/UX_AUDIT_2026.md`'s vision was already shipped in `lib/portal/access.ts` (confirmed via that doc's own M0.1 status reconciliation, §C2), while the task-oriented proposal is explicitly self-flagged as XL/multi-sprint, needing a design sprint and real user validation neither of which happened. Adopted the live one as canonical; added status notes to the other two docs marking their nav proposals not-adopted (kept the rest of both docs — general UI rules, the remaining phased plan — since only the nav sections conflicted). No sidebar/nav code changes; `lib/portal/access.ts` already matches, with a few small already-justified deltas from the original diagram documented in `docs/UX_AUDIT_2026.md` directly. |
