This folder contains Kubernetes manifests to deploy Situs on TrueNAS SCALE.
...
2. Create a PersistentVolumeClaim or use an existing one named `situs-data`.
...
- `DATABASE_URL` is set to `file:/data/situs.sqlite` and the container mounts the PVC at `/data`.
