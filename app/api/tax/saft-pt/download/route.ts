  ...
  try {
    ...
    if (!nif || !name || !addressDetail || !city || !postalCode) {
      return new NextResponse(
        JSON.stringify({
          error: "Missing required fields: nif, name, addressDetail, city, postalCode",
        }),
    ...
    // Validate NIF
    if (!validateNIF(nif)) {
      return new NextResponse(JSON.stringify({ error: "Invalid Portuguese NIF" }), {
        status: 400,
    ...
    if (!validation.valid) {
      return new NextResponse(JSON.stringify({ error: validation.errors.join("; ") }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
