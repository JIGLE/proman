...
if (!host || !port) {
  if (enforce) {
    console.error(
      "[check-hostport] Missing required environment variable(s): HOSTNAME and/or PORT",
    );
    process.exit(1);
  }
  console.warn(
    "[check-hostport] HOSTNAME and/or PORT not set; skipping strict check. Set PRESTART_CHECK_HOSTPORT=true to enforce.",
  );
...
if (!Number.isInteger(portNum) || portNum <= 0 || portNum > 65535) {
  ...
  console.warn(
    "[check-hostport] Invalid PORT value, but PRESTART_CHECK_HOSTPORT not enabled:",
    port,
  );
  process.exit(0);
}
