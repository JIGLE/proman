...
function getDbPath() {
  ...
  if (!url.startsWith("file:"))
    throw new Error("Only file based DATABASE_URL supported by this script");
...
(async () => {
  ...
  try {
    ...
    for (const p of properties) {
      ...
      db.prepare(
        "INSERT INTO units (id, userId, propertyId, unitNumber, name, rent, status, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))",
    ...
    const tenants = db
      .prepare(
        "SELECT id, userId, propertyId, leaseStart, leaseEnd, rent FROM tenants WHERE propertyId IS NOT NULL",
      )
      .all();
    ...
    for (const t of tenants) {
      ...
      const unit = db
        .prepare("SELECT id FROM units WHERE propertyId = ? LIMIT 1")
        .get(t.propertyId);
      if (!unit) {
      ...
      db.prepare(
        "INSERT INTO leases (id, userId, unitId, tenantId, startDate, endDate, rent, status, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', datetime('now'), datetime('now'))",
    ...
    console.log(
      "Backfill complete. Units created:",
      unitsCreated,
      "Leases created:",
      leasesCreated,
    );
