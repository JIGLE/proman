...
const getDatabaseUrl = () => {
  ...
  console.error(
    "No DATABASE_URL provided. Set DATABASE_URL env var or pass the connection string as the first argument.",
  );
  process.exit(2);
};
...
if (url.startsWith("file:")) {
  ...
  if (!Client) {
    console.error(
      "Postgres client not installed. Install `pg` to check a PostgreSQL database, or use a SQLite DATABASE_URL.",
    );
    process.exit(2);
  }
