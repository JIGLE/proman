  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) return;
        const j = await res.json();
        if (j.ok && j.data && !cancelled) setLatest(j.data);
      } catch (err) {
        // ignore
      }
    })();
    ...
    <div className="bg-yellow-50 border border-yellow-200 text-yellow-900 p-3 rounded-md mb-4 mx-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1">A new version of ProMan is available. <a className="underline" href={latest.html_url || '#'} target="_blank" rel="noreferrer">View release notes</a></p>
        </div>
