...
export default function LeaseManagement(): React.ReactElement {
  ...
  const { state } = ctx ?? {
    state: { tenants: localTenants, properties: localProperties, loading: localLoading },
  };
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="space-y-3">
        {tenants.map((tnt) => {
          const prop =
            properties.find((p) => p.id === tnt.propertyId) ||
            properties.find((p) => p.name === tnt.propertyName);
                  ...
                  <div className="mt-4 text-sm text-[var(--color-muted-foreground)]">
                    ...
                    <ul className="mt-1 space-y-1">
                      {tnt.leaseDocuments.map((d) => (
                        <li key={d.id}>
                          <a
                            href={`/api/tenants/documents?id=${d.id}`}
