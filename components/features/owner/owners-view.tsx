    ...
    try {
      ...
      if (owner.properties && owner.properties.length > 0) {
        owner.properties.forEach((po) => {
          ...
          doc.setFontSize(10);
          doc.text(
            `  Share of Income: ${formatCurrency(ownerIncome)}`,
            30,
            yPos,
          );
          doc.text(
            `  Share of Expenses: ${formatCurrency(ownerExpenses)}`,
            30,
            yPos,
          );
  ...
  const filteredOwners = useMemo(() => {
    return owners.filter((owner) => {
      ...
      const matchesProperty =
        propertyFilter === "all" ||
        (owner.properties &&
          owner.properties.some((p) => p.propertyId === propertyFilter));
            ...
            {filteredOwners.length === 0 ? (
              <div className="col-span-full">
                <Card className="bg-zinc-900 border-zinc-800">
                  <CardContent
                    className={compact ? "p-3 text-center" : "p-8 text-center"}
                  >
