                ...
                <div key={tnt.id} className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <div className="text-xs text-zinc-400">{t('leases.documents') || 'Documents'}: {tnt.leaseDocuments?.length ?? 0}</div>
                    </div>
                    <div className="min-w-[220px]">
                      <LeaseDocumentUploader tenantId={tnt.id} onUploaded={() => { /* could refresh docs */ }} />
                    </div>
                  </div>
