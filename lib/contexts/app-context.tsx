...
const AppContext = createContext<{
  ...
  // Property actions
  addProperty: (
    property: Omit<Property, "id" | "userId" | "createdAt" | "updatedAt">,
  ) => Promise<void>;
  updateProperty: (
    id: string,
    property: Partial<
  ...
  // Template actions
  addTemplate: (
    template: Omit<CorrespondenceTemplate, "id" | "createdAt" | "updatedAt">,
  ) => Promise<void>;
  updateTemplate: (
    id: string,
    template: Partial<CorrespondenceTemplate>,
  ) => Promise<void>;
  ...
  addOwner: (
    owner: Omit<
      Owner,
      "id" | "userId" | "createdAt" | "updatedAt" | "properties"
    >,
  ) => Promise<void>;
  updateOwner: (
    id: string,
    owner: Partial<
      Omit<Owner, "id" | "userId" | "createdAt" | "updatedAt" | "properties">
    >,
  ) => Promise<void>;
...
export function AppProvider({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Client-side automatic DB initialization removed. Operator should perform
      // an explicit one-shot init (Helm Job or POST /api/debug/db/init) when deploying.
      console.debug(
        "Client auto DB-init removed; operator must perform explicit DB init when necessary.",
      );
  ...
  const addProperty = async (
    ...
    try {
      ...
      dispatch({
        type: "SET_PROPERTIES",
        payload: [...state.properties, newProperty],
      });
    } catch (err) {
      const errorMessage =
  ...
  const updateProperty = async (
    ...
    try {
      ...
      dispatch({
        type: "SET_PROPERTIES",
        payload: state.properties.map((p) =>
  ...
  const addTenant = async (
    ...
    if (!userId) throw new Error("User not authenticated");
    try {
      const res = await apiFetch<{ data: Tenant }>(
        "/api/tenants",
        csrfToken,
        "POST",
        tenantData,
  ...
  const addReceipt = async (
    ...
    try {
      ...
      dispatch({
        type: "SET_RECEIPTS",
        payload: [...state.receipts, newReceipt],
      });
    } catch (err) {
      const errorMessage =
  ...
  const addTemplate = async (
    ...
    try {
      const res = await apiFetch<{ data: CorrespondenceTemplate }>(
        "/api/correspondence/templates",
        csrfToken,
        "POST",
        templateData,
      ...
      dispatch({
        type: "SET_TEMPLATES",
        payload: [...state.templates, newTemplate],
      });
    } catch (err) {
      const errorMessage =
  ...
  const updateTemplate = async (
    id: string,
    templateData: Partial<CorrespondenceTemplate>,
  ) => {
    try {
      const res = await apiFetch<{ data: CorrespondenceTemplate }>(
  ...
  const addCorrespondence = async (
    ...
    if (!userId) throw new Error("User not authenticated");
    try {
      const res = await apiFetch<{ data: Correspondence }>(
        "/api/correspondence",
        csrfToken,
        "POST",
        correspondenceData,
      ...
      dispatch({
        type: "SET_CORRESPONDENCE",
        payload: [...state.correspondence, newCorrespondence],
      });
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "Failed to add correspondence";
      showError(errorMessage);
      throw err;
    }
  };
  ...
  const updateCorrespondence = async (
    ...
    try {
      ...
      dispatch({
        type: "SET_CORRESPONDENCE",
        payload: state.correspondence.map((c) =>
          c.id === id ? updatedCorrespondence : c,
        ),
      });
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "Failed to update correspondence";
      showError(errorMessage);
      throw err;
    }
  };
  ...
  const addOwner = async (
    ...
    if (!userId) throw new Error("User not authenticated");
    try {
      const res = await apiFetch<Owner | { data: Owner }>(
        "/api/owners",
        csrfToken,
        "POST",
        ownerData,
  ...
  const addExpense = async (
    ...
    try {
      ...
      dispatch({
        type: "SET_EXPENSES",
        payload: [newExpense, ...state.expenses],
      });
    } catch (err) {
      const errorMessage =
  ...
  const addMaintenance = async (
    ...
    try {
      const res = await apiFetch<
        MaintenanceTicket | { data: MaintenanceTicket }
      >("/api/maintenance", csrfToken, "POST", ticketData);
      const newTicket = (res as { data: MaintenanceTicket }).data ?? res;
      dispatch({
        type: "SET_MAINTENANCE",
        payload: [newTicket, ...state.maintenance],
      });
    } catch (err) {
      const errorMessage =
  ...
  const updateMaintenance = async (
    id: string,
    ticketData: Partial<MaintenanceTicket>,
  ) => {
    if (!userId) throw new Error("User not authenticated");
    try {
      const res = await apiFetch<
        MaintenanceTicket | { data: MaintenanceTicket }
      >(`/api/maintenance/${id}`, csrfToken, "PUT", ticketData);
      const updatedTicket = (res as { data: MaintenanceTicket }).data ?? res;
      dispatch({
        type: "SET_MAINTENANCE",
        payload: state.maintenance.map((t) =>
  ...
  const addLease = async (
    ...
    try {
      const newLease = await apiFetch<Lease>(
        "/api/leases",
        csrfToken,
        "POST",
        leaseData,
  ...
  const updateLease = async (id: string, leaseData: Partial<Lease>) => {
    try {
      const updatedLease = await apiFetch<Lease>(
        `/api/leases/${id}`,
        csrfToken,
        "PUT",
        leaseData,
      ...
      dispatch({
        type: "SET_LEASES",
        payload: state.leases.map((l) => (l.id === id ? updatedLease : l)),
      });
    } catch (err) {
