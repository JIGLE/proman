...
system in `app/globals.css`, the 4-zone modal). This loop raises *execution* until
...
override a defensible alternative get one line of *why* in the commit body.
...
| **Motion restraint** | Purposeful only; calm easing; no bounce/elastic; one animation system per surface (CSS keyframes *or* Framer, not both) |
...
- **Shared primitives break every consumer.** `components/ui/sheet.tsx` hardcoding
  `bg-zinc-900` broke *every* Sheet-based modal in light/OLED. Fix primitives first.
