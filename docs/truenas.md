...
This guide covers deploying Situs on TrueNAS SCALE.

## Quick Install

1. In TrueNAS Apps UI, select **Custom App** (or install from a catalog).
2. Set the container image: `ghcr.io/jigle/situs:<version>`.
...
| **Storage Path**    | `/mnt/pools/mypool/apps/situs/data` | TrueNAS dataset for the SQLite DB |
...
- **DATABASE_URL** defaults to `file:/app/data/situs.db`.
...
vim helm/situs/values-truenas.yaml

# 2. Install
helm install situs helm/situs -f helm/situs/values-truenas.yaml
```
...
persistence:
  hostPath: /mnt/pools/mypool/apps/situs/data
```
...
2. Create dataset: `apps/situs/data`
...
`situs-secrets`. Retrieve it with:

```bash
kubectl get secret situs-secrets -n ix-situs \
...
```bash
kubectl get pods -n ix-situs
kubectl exec -it <pod-name> -n ix-situs -- npx prisma db push --schema=prisma/schema.prisma
```

## Updating the App

1. Check for new releases at https://github.com/JIGLE/situs/releases
...
1. Check pod logs: `kubectl logs -l app=situs -n ix-situs --tail=100`
...
3. Verify the data directory is writable by UID 1001:
   ```bash
   kubectl exec -it <pod> -n ix-situs -- ls -la /app/data
   ```
...
- Check init container logs — the `prisma-init` init container may have failed:
  ```bash
  kubectl logs <pod-name> -c prisma-init -n ix-situs
  ```
- Verify the dataset path is correct and writable
- Check main container logs: `kubectl logs -l app=situs -n ix-situs --tail=100`

**Cannot access the app after install:**

- Check NodePort: `kubectl get svc -n ix-situs`
- Try port-forward: `kubectl port-forward svc/situs 3000:80 -n ix-situs`

**Permission denied / SQLite not writable:**

- Ensure the TrueNAS dataset permissions are set to UID `1001`, GID `1001`.
- Verify: `kubectl exec -it <pod> -- stat /app/data/situs.db`
...
3. Set **Authorized JavaScript origins**: `https://situs.example.com`
4. Set **Authorized redirect URIs**: `https://situs.example.com/api/auth/callback/google`
...
```bash
# Via Helm
helm uninstall situs -n ix-situs
