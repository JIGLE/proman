...
| F3  | **"Try Demo Mode" is broken.** The proxy's page auth-guard doesn't bypass for the `situs_demo` cookie, so the demo entry bounces to `/auth/signin`.                                                                                                       | `proxy.ts` (portal auth-guard)                                                                              | Treat a valid demo cookie as authenticated for portal pages (or issue a scoped demo token).                                |
