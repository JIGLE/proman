...
| **Wrong dev-branch pointer** | `CLAUDE.md:76` → `claude/determined-clarke-swzwsz`. **This row's own fix was wrong**: it named `claude/situs-design-polish-6zpz2f`, a branch that has never existed on the remote. The real one is `claude/proman-design-polish-6zpz2f`, and the bad name was copied into `CLAUDE.md` and `ROADMAP.md` from here (corrected 2026-08-17). A wrong remedy recorded in an audit propagates exactly as fast as a right one. | Point `CLAUDE.md` at `claude/proman-design-polish-6zpz2f`                        |
