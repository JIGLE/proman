...
lib/
  ...
  services/
    ...
    bank/               # CSV import + fingerprint dedupe + matching pipeline
      providers/        # PSD2 provider contract + registry + test fake (no adapter ships)
...
No bank data provider ships, so there is nothing to configure for one and the app is
CSV-import-only. `CRON_SECRET` gates all three `/api/cron/*` endpoints (notifications, data retention, bank sync);
