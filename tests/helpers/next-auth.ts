...
export async function mockGetServerSession(value: Session | null): Promise<void> {
  const mod = (await import("next-auth")) as unknown as {
    getServerSession: (...args: unknown[]) => Promise<Session | null>;
  };
  const modNext = (await import("next-auth/next")) as unknown as {
    getServerSession: (...args: unknown[]) => Promise<Session | null>;
  };
