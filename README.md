...
| `RUN_PRISMA_DB_PUSH_AT_STARTUP`    | Optional    | When `true`, runs `npx prisma db push && npx prisma generate` at container startup (not recommended in production; prefer init job).        |
